...
<div class="mui-content">
	<article class="mt10 jiekuan">
		...
		<div class="cf okdan">
			<div class="oktable">
				...
				<span>￥<?php echo ($order["money"]); ?>.00元</span>
			</div>
			<div class="oktable">
				<span class="fc9 listit">借款期限</span>
				<span><?php echo ($order["month"]); ?>月 </span>
			</div>
			<div class="oktable">
				<span class="fc9 listit">每月还款</span>
				<span>¥<?php echo $order['huankuan']+$order['fuwufei'] ?>.00元
				</span>
				
			</div>
			<div class="oktable">
				<span class="fc9 listit"></span>
				<span class="fc9">（含日利率 <?php echo ($order["rixi"]); ?>% ¥<?php echo ($order["fuwufei"]); ?>.00元）</span>
			</div>
			<div class="oktable">
			...
			<div class="oktable">
				...
				<span><?php echo C('cfg_shenhefei');?>.00元</span>
			</div>
			 <!--<div class="oktable">
 ...
 $name = "审核费用支付协议"; $Block = D("block"); $blockinfo = $Block->where(array('name' => $name))->find(); if(empty($blockinfo)){ echo ""; }else{ echo $blockinfo['cont']; } ?>
			...
			<div class="protit sevagreee " style="background-color:#f5f5f9;">
	            <a class="logBtn" href="javascript:subForm();">申请</a>
	        </div>
</div>
				    	...
				    	function (data,state){
				    		if(state != "success"){
						     ...
						     //    setTimeout(function () { 
							    ...
							    // }, 2500);
				    			 mui.alert('你的申请已经受理！可在我的借款中跟进借款审核进度！', '提示信息', function() 
							    {
							        window.location.href = data.payurl;
							    });
				    		}
